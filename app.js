...
const novoArrayImpar = randomNumbers.filter(getOddNumbers)

console.log(novoArrayImpar)
...
const countNumberLessThan501 = (acumulator, crazyNumber) => 
  crazyNumber < 501 ? ++acumulator : acumulator

const abaixo501 = crazyNumbers.reduce(countNumberLessThan501, 0)
...
//poderia ser também number ** 2
const numerosAoQuadrado = numbers.map(number => number * number)
...
const listaJogos = cart.reduce((acumulator, {name})=>
  `${acumulator}- ${name}\n`, '')
